...
describe('link target', () => {
  ...
  it('should render a quick reply with a link to google targeting self', () => {
    ...
    expect(quickReplyComponent.find('a.push-reply')).toHaveLength(1);
    expect(quickReplyComponent.find('a.push-reply').text()).toEqual('google');
    expect(quickReplyComponent.find('a.push-reply').prop('href')).toEqual('http://www.google.ca');
    expect(quickReplyComponent.find('a.push-reply').prop('target')).toEqual('_self');
  });
  ...
  it('should render a quick reply with a link to google targeting blank', () => {
    ...
    expect(quickReplyComponent.find('a.push-reply').text()).toEqual('google');
    expect(quickReplyComponent.find('a.push-reply').prop('href')).toEqual('http://www.google.ca');
    expect(quickReplyComponent.find('a.push-reply').prop('target')).toEqual('_blank');
  });
});
