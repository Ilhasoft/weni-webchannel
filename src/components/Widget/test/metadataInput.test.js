...
describe('Metadata store affect input behavior', () => {
  ...
  it('should disable the input', () => {
    expect(senderCompoment.find('.push-new-message')).toHaveLength(1);
    expect(senderCompoment.find('.push-new-message').prop('disabled')).toEqual(false);
    store.dispatch({ type: 'SET_USER_INPUT', userInputState: 'disable' });
    senderCompoment.update(); // propagate new store to the compoment
    expect(senderCompoment.find('.push-new-message')).toHaveLength(1);
    expect(senderCompoment.find('.push-new-message').prop('disabled')).toEqual(true);
  });
  ...
  it('should hide the input', () => {
    ...
    senderCompoment.update(); // propagate new store to the compoment
    expect(senderCompoment.find('.push-new-message')).toHaveLength(0);
  });
});
