...
describe('Message custom Css', () => {
  ...
  it('check that a custom style is correctly applied', () => {
    ...
    expect(messageComponent.find('div.push-response').prop('style')).toEqual({ cssText: 'color:red;' });
    expect(messageComponent.find('div.push-response').prop('className')).toEqual('push-response');
  });
  ...
  it('check that a class style is correctly applied', () => {
    ...
    expect(messageComponent.find('div.push-response').prop('className')).toEqual('push-response test-class');
    expect(messageComponent.find('div.push-response').prop('style')).toEqual({ cssText: undefined });
  });
  ...
  it('check that a  style cannot be applied to a user message', () => {
    ...
    expect(messageComponent.find('div.push-client').prop('className')).toEqual('push-client');
    expect(messageComponent.find('div.push-client').prop('style')).toEqual({ cssText: undefined });
  });
});
