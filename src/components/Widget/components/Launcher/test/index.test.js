...
describe('<Launcher />', () => {
  ...
  it('should call toggle prop when clicked', () => {
    ...
    launcherComponent.find('.push-launcher').simulate('click');
  ...
  describe('Rendering open-launcher image when chatOpened = false', () => {
    ...
    it('should not display unreadCount when count is 0', () => {
      ...
      expect(launcherComponent.find('.push-open-launcher')).toHaveLength(1);
      expect(launcherComponent.find('.push-close-launcher')).toHaveLength(0);
      expect(launcherComponent.find('.push-unread-count-pastille')).toHaveLength(0);
    });
    ...
    it('should display unreadCount when count is superior to 0 and displayUnreadCount = true', () => {
      ...
      expect(launcherComponent.find('.push-open-launcher')).toHaveLength(1);
      expect(launcherComponent.find('.push-close-launcher')).toHaveLength(0);
      expect(launcherComponent.find('.push-unread-count-pastille')).toHaveLength(1);
    });
  });
  ...
  it('should render the close-launcher image when chatOpened = true', () => {
    ...
    const launcherComponent = createLauncherComponent({ toggle, chatOpened });
    expect(launcherComponent.find('.push-open-launcher')).toHaveLength(0);
    expect(launcherComponent.find('.push-close-launcher')).toHaveLength(1);
  });
});
